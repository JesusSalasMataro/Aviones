...
public class clEspacioAereo {
	...
	private int iAvionActivo = -1;  // Indica el n�mero de avi�n activo al que se le aplicar�n
	...
	// Devuelve el �ndice del panel asignado si se ha podido incorporar el avi�n
	// al espacio a�reo, devuelve -1 en caso contrario
	...
	// de los l�mites del espacio a�reo
			...
			if( aviAviones[i] != null ) {
				for( ii=i; ii<clEspacioAereo.MAX_AVIONES; ii++ ) {
					if( (aviAviones[ii] != null) && (ii != i) && !bAccidente ) {
						...
						// Comprovamos si hay 2 aviones que est�n demasiado cerca, tanto en altura como en distancia entre ellos,
						...
						// espacio a�reo los aviones implicados
						if( (iDiferenciaAltitud < ALTITUD_ACCIDENTE) 
								...
								clVentanaPrincipal.lblMensajes.append("ACCIDENTE a�reo entre aviones " + 
									Integer.toString(i) + " y " + Integer.toString(ii) + ".\n");
								clVentanaPrincipal.lblMensajes.setCaretPosition(clVentanaPrincipal.lblMensajes.getDocument().getLength());
								eliminarAvion(i);
								eliminarAvion(ii);
	...
	// Esta funci�n se encarga de establecer el nuevo avi�n activo despu�s de haber
	// eliminado un avi�n
	...
	// Cambia de avi�n activo al siguiente al pulsar el bot�n "Cambiar avi�n"
	...
	// Dada una matr�cula devuelve un avi�n en caso de que exista o nulo en
	// caso de no existir
	...
	// Comprueba si una determinada posici�n de la pista est� libre
