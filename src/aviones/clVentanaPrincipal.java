...
public class clVentanaPrincipal extends JFrame {
	...
	public clVentanaPrincipal() {
		...
		// Im�genes del programa: fondos, iconos de los aviones, etc.
		...
		// Panel que representa el espacio a�reo en la pantalla, es el espacio m�s
		...
		// EspacioAereo contendr� los aviones hasta un m�ximo de 5
		EspacioAereo = new clEspacioAereo();
		bSimuladorDetenido = true;
		
		
		// Panel contenedor de los paneles de informaci�n de los aviones en la parte
		...
		JButton btnNuevoAvion = new JButton("Nuevo avi�n");
		...
		btnNuevoAvion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( EspacioAereo.espacioAereoLibre() ) {
					...
					strFabricante = JOptionPane.showInputDialog(contentPane, "Fabricante:", "Nuevo avi�n", JOptionPane.QUESTION_MESSAGE);				
					if( strFabricante != null ) {
						
						strModelo = JOptionPane.showInputDialog(contentPane, "Modelo:", "Nuevo avi�n", JOptionPane.QUESTION_MESSAGE);
						
						if( strModelo != null ) {
													
							iCapacidad = 0;
							iPosX = -1; iPosY = -1;
							// La matr�cula es obligatoria para dar de alta un nuevo avi�n
							try {
								while( strMatricula.equals("") )
									strMatricula = JOptionPane.showInputDialog(contentPane, "Matr�cula:", "Nuevo avi�n", JOptionPane.QUESTION_MESSAGE);
							} catch( Exception e ) {  }  // La matr�cula es nula
							
							if( strMatricula != null ) {
								
								try {
									iCapacidad = Integer.parseInt(JOptionPane.showInputDialog(contentPane, "Capacidad:", "Nuevo avi�n", JOptionPane.QUESTION_MESSAGE));
									while( (iPosX < 0) || (iPosX > ANCHO_ESPACIO_AEREO) ) {
										iPosX = Integer.parseInt(JOptionPane.showInputDialog(contentPane, "Posici�n eje X:", "Nuevo avi�n", JOptionPane.QUESTION_MESSAGE));
									}
									while( (iPosY < 0) || (iPosY > ALTO_ESPACIO_AEREO) ) {						
										iPosY = Integer.parseInt(JOptionPane.showInputDialog(contentPane, "Posici�n eje Y:", "Nuevo avi�n", JOptionPane.QUESTION_MESSAGE));
									}
								} catch( Exception e ) {
									bDatosCorrectos = false;
									lblMensajes.append("No se ha creado el avi�n. Datos no v�lidos.\n");
								}
					...
					lblMensajes.append("Espacio a�reo lleno.\n");
		...
		JButton btnEliminarAvion = new JButton("Eliminar avi�n");
		...
		btnAcelerar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( !aviAvionActual.motorEncendido() ) { 
					lblMensajes.append("El avi�n no tiene el motor encendido.\n");
				}
				else {
		...
		btnFrenar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				clAvion aviAvionActual = new clAvion();
				aviAvionActual = EspacioAereo.getAvion(EspacioAereo.getAvionActivo());
				if( aviAvionActual.getVelocidad() == 0  ) lblMensajes.append("El avi�n est� parado.\n");
				else { 
		...
		btnAscender.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( iElevar == aviAvionActual.ELEVACION_OK ) {
					...
					lblMensajes.append("El motor est� apagado.\n");			
		...
		btnDescender.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if( iDescender == aviAvionActual.DESCENSO_OK ) {
					...
					lblMensajes.append("El avi�n ya se encuentra a altitud 0.\n");			
				...
				while( ((iRumbo < 0) || (iRumbo > 360)) && (strRumbo != null) ) {
					...
					if( strRumbo != null ) {
						...
						if( (iRumbo < 0) || (iRumbo > 360) )
							lblMensajes.append("Valor no v�lido, ha de ser entre 0 y 360 grados.\n");
					}
		...
		// Panel de visualizaci�n de los mensajes que emite el programa para comunicarse con el usuario,
